...
export function showPolyominoPopup(toolbar) {
  ...
  const shapeSize = 24;
  const padding = 90;
  const requiredHeight = Math.max(1400, 120 + shapes.length * (shapeSize + padding));
  const popupContainer = toolbar.createPopupContainer(
    'polyominoPopup',
    toolbar.buttons[0].name,
    requiredHeight
  );
  ...
  popup.addEventListener('mousemove', (e) => {
    ...
    polyominoes.forEach(({ polyomino, y }) => {
      ...
      const rightColumnRect = {
        x: polyomino.x,
        y: polyomino.y,
        width: polyomino.width,
        height: polyomino.height,
      };
      if (
        toolbar.isInside(mouseX, mouseY, leftColumnRect) ||
        toolbar.isInside(mouseX, mouseY, rightColumnRect)
      ) {
        cursor = 'pointer';
      }
    });
  ...
  popup.addEventListener('click', (e) => {
    ...
    const canvasPaddingX =
      getRandomPosition(window.innerWidth < 600 ? 300 : 1200) + (window.innerWidth < 600 ? 0 : 45);
