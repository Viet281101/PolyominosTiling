...
class MainApp {
  ...
  drawPolyominoes() {
    this.polyominoes.forEach((polyomino) =>
      polyomino.draw(this.gridBoard.ctx, this.gridSize, this.selectedPolyomino === polyomino)
    );
  }
  ...
  addEventListeners() {
    ...
    this.canvas.addEventListener('touchstart', (e) => {
      e.preventDefault();
      this.handleMouseDown(this.gridBoard.getTouchPos(e));
    });
    this.canvas.addEventListener('touchmove', (e) => {
      e.preventDefault();
      this.handleMouseMove(this.gridBoard.getTouchPos(e));
    });
  ...
  autoRandomBlackening() {
    ...
    const minBlackCells = Math.floor(totalCells * 0.2);
    const maxBlackCells = Math.floor(totalCells * 0.25);
    const nbBlackCellsWanted =
      Math.floor(Math.random() * (maxBlackCells - minBlackCells + 1)) + minBlackCells;
  ...
  createMessageBox(type_tiling) {
    ...
    switch (type_tiling) {
      ...
      case 4:
        ...
        break;
      case 5:
        messageBox.textContent = 'FULL AUTOMATIC TILING FINISHED';
        break;
      default:
