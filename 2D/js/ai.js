...
export function backtrackingAutoTiling(polyominoes, gridBoard, placePolyomino, removePolyomino) {
	...
	if (tryPlacePolyominoesBacktracking(0, polyominoesCopy, gridBoard, placePolyomino, removePolyomino)) {
...
function tryPlacePolyominoesBacktracking(index, polyominoes, gridBoard, placePolyomino, removePolyomino) {
	...
	for (let row = 0; row < gridBoard.rows; row++) {
		for (let col = 0; col < gridBoard.cols; col++) {
			...
			if (gridBoard.isInBounds(polyomino) && !gridBoard.isOverlapping(polyomino)) {
				...
				if (tryPlacePolyominoesBacktracking(index + 1, polyominoes, gridBoard, placePolyomino, removePolyomino)) {
					return true;
				}
